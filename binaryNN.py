...
x = binary_layer(x, units=64)
pred = binary_layer(x, units=10)

loss = tf.losses.softmax_cross_entropy(outputs, pred)
tf.summary.scalar('loss', loss)

l_r = 1e-3
train = tf.train.AdamOptimizer(learning_rate=l_r)
...
with tf.Session() as sess:
    ...
    for m in range(epochs):
        ...
        for i in range(iterations):
            ...
            _, loss_train, summary = sess.run([train1, loss, merged], {inputs: batch_x, outputs: batch_y})
